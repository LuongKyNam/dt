		...
		this.LoadArrayBuffer = function ( e ) {
			...
			if ( q.is_ready )
			{
					...
					new PKSimpleModal ({
						...
						body    : '<p>Nối tập tin vào bản nhạc hiện có?</p>',
						setup   : function( q ) {
...
//			audio_el.playbackRate = 0.5;
		...
		wavesurfer.on ('ready', function () {
			...
			setTimeout(function () {OneUp ('Đã tải thành công')}, 180);
...
//		});
		...
		app.listenFor ('RequestActionFX_HardLimit', function ( val ) {
			...
			OneUp ('Giới hạn cứng được áp dụng (fx)');
		});
		...
		app.listenFor ('RequestActionFX_Flip', function ( val, val2 ) {
			...
			OneUp ('Thay đổi kênh được áp dụng: ' + val);
		});
...
*/
		...
		app.listenFor ('StateDidPop', function ( state, undo ) {
			...
			if (undo) OneUp ('Hoàn tác ' + state.desc);
			else OneUp ('Làm lại ' + state.desc);
		});
