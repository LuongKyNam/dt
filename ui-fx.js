	...
	function PKUI_FX ( app ) {
		...
		app.listenFor ('RequestFXUI_Gain', function () {
			...
			var x = new PKAudioFXModal({
				id: filter_id,
			    title:'Áp dụng Tăng cho phạm vi đã chọn',
			...
			preview: function ( q ) {
			  ...
			  body:'<div class="pk_row" style="border:none"><label>Đạt được phần trăm</label>' + 
		...
		app.listenFor ('RequestActionFXUI_Speed', function () {
			...
			var x = new PKAudioFXModal({
			  ...
			  title:'Thay đổi tốc độ',
				presets:[
					{name:'-1/4',val:0.25},
					{name:'-1/2',val:0.5},
					{name:'Chậm hơn một chút',val:0.85},
					{name:'Nhanh hơn một chút%',val:1.1},
					{name:'+1/4',val:1.25},
					{name:'+1/2',val:1.5}
				],
				custom_pres:custom_presets.Get (filter_id),
			...
			preview: function ( q ) {
			  ...
			  body:'<div class="pk_row" style="border:none"><label>Tốc độ phát lại</label>' + 
			...
			buttons: [
			  ...
			  body:'<div class="pk_row pk_mm" style="border:none;display:none">'+
					...
					'<div class="pk_row pk_ms" style="border:none;display:none">'+
						...
						'<label for="xms">Làm Stereo</label></div>' + 
					'</div>',
			  setup:function( q ) {
			  	  var main = null;
		...
		app.listenFor ('RequestFXUI_Silence', function () {
			var x = new PKSimpleModal({
			  ...
			  body:'<div class="pk_row"><input type="radio" class="pk_check" id="ifeq" name="rdslnc" value="beginning">'+ 
				'<label  for="ifeq">Chèn khoảng lặng ở đầu</label><br/>' +
				'<input type="radio" class="pk_check"  id="vgdja" name="rdslnc" checked value="cursor">'+
				'<label for="vgdja">Chèn khoảng lặng vào con trỏ hiện tại (<span class="pkcdpk"></span>)</label></div>'+
				'<div class="pk_row"><label>Im lặng trong vài giây</label>'+
		...
		app.listenFor ('RequestActionFXUI_Compressor', function () {
			...
			var x = new PKAudioFXModal({
			  id    : filter_id,
			  title : 'Áp dụng nén cho phạm vi đã chọn',
			  clss  : 'pk_bigger',
			ondestroy: function ( q ) {
				app.ui.InteractionHandler.on = false;
				app.ui.KeyHandler.removeCallback (modal_esc_key);
				...
				presets:[
					{name:'Cổ điển',val:'-40,5,7,0.002,0.1'},
					{name:'Ánh sáng',val:'-6,2,2.5,0.002,0.05'},
					{name:'Biến dạng nét đứt',val:'-45,26,2.05,0.233,0.0'},
					{name:'Sự biến dạng hỗn loạn',val:'-60,14,11.07,0.036,0.00'}
				],
				custom_pres:custom_presets.Get (filter_id),
			preview: function ( q ) {
				var inputs = q.el_body.getElementsByTagName('input');
		...
		app.listenFor ('RequestActionFXUI_Normalize', function () {
			...
			var x = new PKSimpleModal({
			  ...
			  body:'<div class="pk_row">'+
				...
				'<label for="vhcjgs">Bình thường hóa L/R như nhau</label></div>' + 
				'<div class="pk_row" style="border:none"><label>Bình thường hóa thành</label>'+
		...
		app.listenFor ('RequestActionFXUI_HardLimiter', function () {
			...
			var x = new PKAudioFXModal({
			  ...
			  body:
				...
				'<label for="xighs">Giới hạn ở</label></div>' + 

				'<div class="pk_row"><label>Giới hạn ở</label>'+
				'<input type="range" min="0.1" max="1.0" class="pk_horiz pk_w180" step="0.01" value="0.99" />'+
				'<span class="pk_val">99%</span></div>'+

				'<div class="pk_row"><label>Tỷ lệ giữa mức thấp và mức cao</label>'+
				'<input type="range" min="0.0" max="1.0" class="pk_horiz pk_w180" step="0.01" value="0.0" />'+
				'<span class="pk_val">Tỷ lệ 0%</span></div>'+

				'<div class="pk_row"><label>Nhìn về phía trước (ms)</label>'+
		...
		app.listenFor ('RequestActionFXUI_Delay', function () {
			...
			var x = new PKAudioFXModal({
			  id    : filter_id,
			  title : 'Áp dụng Độ trễ cho phạm vi đã chọn',
			  clss  : 'pk_bigger',
			ondestroy: function ( q ) {
				app.ui.InteractionHandler.on = false;
				app.ui.KeyHandler.removeCallback (modal_esc_key);
				...
				presets:[
					{name:'Cổ điển',val:'0.3,0.4,0.4'},
					{name:'Không gian',val:'3.0,0.6,0.3'}
				],
				custom_pres:custom_presets.Get (filter_id),
			preview: function ( q ) {
				var val = getvalue (q);
		...
		app.listenFor ('RequestActionFXUI_Distortion', function () {
			...
			var x = new PKAudioFXModal({
			  id    : filter_id,
			  title : 'Áp dụng biến dạng cho phạm vi đã chọn',
			  clss  : 'pk_bigger',
			ondestroy: function ( q ) {
				app.ui.InteractionHandler.on = false;
				app.ui.KeyHandler.removeCallback (modal_esc_key);
			...
			preview: function ( q ) {
			  ...
			  body:'<div class="pk_row"><label class="pk_line">Tăng</label>' + 
				'<input class="pk_horiz" type="range" min="0.0" max="2.0" step="0.01" value="0.5" />'+
				'<span class="pk_val">0.5</span></div>',
				// '<a style="float:left;margin:0" class="pk_modal_a_bottom">Biểu đồ khối lượng</a></div>',
		...
		app.listenFor ('RequestActionFXUI_Reverb', function () {
			...
			var x = new PKAudioFXModal({
			  id    : filter_id,
			  title : 'Áp dụng hồi âm cho phạm vi đã chọn',
			  clss  : 'pk_bigger',
			ondestroy: function ( q ) {
				app.ui.InteractionHandler.on = false;
				app.ui.KeyHandler.removeCallback (modal_esc_key);
			...
			preview: function ( q ) {
			  ...
			  body:'<div class="pk_row"><label class="pk_line">Thời gian</label>' + 
				'<input class="pk_horiz" type="range" min="0.0" max="3.0" step="0.01" value="0.3" />'+
				'<span class="pk_val">0.3</span></div>'+

				'<div class="pk_row"><label class="pk_line">Phân hủy</label>' + 
		...
		app.listenFor ('RequestActionID3', function (flag, new_tags) {
				...
				var render_tags = function ( el, tags ) {
					...
					str += '<div><span class="pk_id3ttl">Nghệ sĩ</span><span>' + (tags.artist || '-') + '</span></div>';
					str += '<div><span class="pk_id3ttl">Tiêu đề</span><span>' + (tags.title || '-') + '</span></div>';
					str += '<div><span class="pk_id3ttl">Album</span><span>' + (tags.album || '-') + '</span></div>';
					str += '<div><span class="pk_id3ttl">Năm</span><span>' + (tags.year || '-') + '</span></div>';
					str += '<div><span class="pk_id3ttl">Thể loại</span><span>' + (tags.genre || '-') + '</span></div>';
					str += '<div style="max-width:700px"><span class="pk_id3ttl">Bình luận</span><span>' + ((tags.comment||{}).text || '-') + '</span></div>';
					str += '<div><span class="pk_id3ttl">Track</span><span>' + (tags.track || '-') + '</span></div>';
					str += '<div style="max-width:700px"><span class="pk_id3ttl">Lời bài hát</span><span>' + ((tags.lyrics||{}).lyrics || '-') + '</span></div>';
				...
				new PKSimpleModal({
				  title:'Trình khám phá thẻ meta ID3',
				  ...
				  body:'<input type="file" accept="audio/*" />'+
				  	'<div class="pk_row pk_ttx">Chọn tệp để xem thẻ meta âm thanh!</div>',
				  setup:function( q ) {
				  		var input  = q.el_body.getElementsByTagName ('input')[0];
				  		...
				  		input.onchange = function ( e ) {
							...
							reader.onload = function() {
								...
								if (!tags) {
									txt_el.innerHTML = '<div style="padding:30px 0">Không tìm thấy siêu dữ liệu âm thanh...</div>';
								} else {
		...
		app.listenFor ('RequestSavePreset', function () {
			...
			if (preset_obj.val.length > 0)
			{
					...
					if (!is_new)
					{
							...
							btn_delete = {
									...
									callback: function( q ) {
										...
										OneUp ('Đã xóa cài đặt trước thành công!', 1400);
							...
							btn_update = {
									...
									callback: function( q ) {
										...
										if (custom_obj)
										{
											...
											if (value.length > 0)
											{
												OneUp ('Đã cập nhật cài đặt trước thành công!', 1400);
					...
					var title = 'Lưu cài đặt trước tùy chỉnh cho bộ lọc "' + curr_filter_ui.id + '"';
					if (!is_new) {
						var cname = custom_obj.name;
						title = 'Chỉnh sửa cài đặt trước tùy chỉnh "' + cname + '", for filter "' + curr_filter_ui.id + '"';
					}
					...
					new PKSimpleModal({
					  ...
					  buttons:[
						{
							title: is_new ? 'Lưu' : 'Lưu dưới dạng mới',
							clss:'pk_modal_a_accpt',
							callback: function( q ) {
								var input = q.el_body.getElementsByTagName ('input')[0];
								var value = input.value.trim ();
								...
								if (value.length > 0)
								{
									OneUp ('Đã lưu cài đặt trước thành công!', 1400);
					  ...
					  body:'<label for="k07">Tên đặt trước</label>' + 
						'<input style="min-width:340px" maxlength="16" placeholder="Vui lòng nhập tên, ví dụ: Bản nhạc của tôi" ' + default_txt + ' class="pk_txt" type="text" id="k07" />',
					  setup:function( q ) {
					  	  	// app.fireEvent ('RequestPause');
		...
		app.listenFor ('RequestDragI', function ( url ) {
			if (app.isMobile) {
				alert ('không được hỗ trợ trên thiết bị di động');
				return ;
			}
		...
		app.listenFor ('RequestShowFreqAn', function ( url, args_arr ) {
			...
			if (app.isMobile) {
				alert ('Hiện không được hỗ trợ trên thiết bị di động');
				return ;
			}

			var toggle = args_arr[ 0 ];
			var type   = args_arr[ 1 ];
			var title = 'Phân tích tần số';
			var curr_win = eq_win[ url ];

			if (url === 'sp') title = 'Phân tích phổ';
			...
			if (!type)
			{
				var makePopup = function ( dat ) {
					...
					if (!wnd) {
						OneUp ('Vui lòng cho phép cửa sổ bật lên LGV91 Editor Audio Mixer!', 3600, 'pk_r');
						return ;
					}
