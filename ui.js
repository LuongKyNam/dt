(function ( w, d, PKAE ) {
	...
	var PKUI = function( app ) {
		...
		app.listenFor ('ShowError', function( message ) {
			new PKSimpleModal ({
				title : 'Ối! Có điều gì đó không đúng',
				clss:'pk_modal_anim',
		...
		return [
				...
				children : [
					{
						...
						action: function () {
								new PKSimpleModal({
								  ...
								  body:'<div class="pk_row"><label for="k0">Tên tệp</label>' + 
									'<input style="min-width:250px" placeholder="mp3 filename" value="audiomass-output.mp3" ' +
									'class="pk_txt" type="text" id="k0" /></div>'+

									'<div class="pk_row" id="frmtex" style="padding-bottom:4px"><label style="display:inline">Định dạng</label>'+
									...
									'<label for="k4">Xuất toàn bộ tập tin</label>'+
									'<input type="radio" class="pk_check" id="k5" name="xport" value="sel">'+
									'<label class="pk_lblmp3" for="k5">Chỉ xuất lựa chọn</label></div>',
					...
					
					{
						...
						action: function ( e ) {
								new PKSimpleModal({
								  title:'Tải âm thanh từ url từ xa',
								  ...
								  buttons:[
									{
										...
										callback: function( q ) {
											...
											{
												OneUp ('Đã nhập URL không hợp lệ', 1100);
											}
											// -
										}
									}
								  ],
								  body:'<label for="k00">Chèn url</label>' + 
									'<input style="min-width:250px" placeholder="Vui lòng chèn url" class="pk_txt" type="text" id="k00" />',
								  setup:function( q ) {
...

					{
						...
						action: function ( e ) {
							...
							new PKSimpleModal ({
								...
								body:'<p>Hãy chọn nguồn...</p>' +
									'<div class="pk_row"><input type="radio" class="pk_check" id="sl1" name="rdslnc" checked value="whole">'+ 
									'<label style="vertical-align:top" for="sl1">Toàn bộ bản nhạc</label>' +
									'<input type="radio" class="pk_check"  id="sl2" name="rdslnc" value="sel">'+
									'<label style="vertical-align:top" class="pk_lblsel" for="sl2">Lựa chọn'+
									...
									'<div class="pk_row"><label for="slk0">Tên dự thảo</label>' + 
						...
						setup: function ( obj ) {
							...
							app.listenFor ('DidStoreDB', function ( obj, e ) {
									...
									new PKSimpleModal ({
										...
										body:'<p>Mở trong cửa sổ mới?</p>' + txt,
										setup:function( q ) {
											app.fireEvent ('RequestPause');
											app.fireEvent( 'RequestRegionClear');
						...
						action: function ( e ) {
							...
							var func = function ( fls ) {								
								fls.ListSessions(function( ret ) {
									...
									var msg = '';
									if (ret.length === 0) {
										msg += 'Không tìm thấy bản nháp nào...';
									}
									else
									{
										for (var i = 0; i < ret.length; ++i)
										{
											...
											'<a class="pk_lcla2" onclick="PKAudioEditor.fireEvent(\'LoadDraft\',\'' + curr.id + '\', 3);">CHƠI</a>' +
											'<a class="pk_lcla" onclick="PKAudioEditor.fireEvent(\'LoadDraft\',\'' + curr.id + '\');">Mở</a>';

											if (app.engine.is_ready) {
												msg += '<a onclick="PKAudioEditor.fireEvent(\'LoadDraft\',\'' + curr.id +
												 '\',1);" class="pk_lcla">Nối vào bản nhạc hiện tại</a>';
											}
									...
									app.fls.Init (function (err) {
										...
										if (app.engine.is_ready && !append)
										{
											var mm = new PKSimpleModal ({
												title : 'Mở trong hiện có?',
												body  : '<div>Mở trong cửa sổ mới hay trong cửa sổ hiện tại?</div>',
												buttons:[
													{
														title:'MỞ',
														clss:'pk_modal_a_accpt',
			...
			},
			{
				...
				children:[
					...
					{
						name: 'Chơi <span class="pk_shrtct">Space</span>',
						action: function () {
					...
					{
						name: 'Chọn tất cả <span class="pk_shrtct">Shft+A</span>',
						action: function () {
					...
					{
						name: 'Bỏ chọn tất cả <span class="pk_shrtct">~</span>',
						action: function () {
			...
			},
			{
				...
				children:[
...

					{
						...
						setup: function ( obj ) {
							app.listenFor ('DidToggleFreqAn', function ( url, val ) {
								...
								var txt = 'Máy phân tích quang phổ';
								if (val) {
									obj.innerHTML = txt + ' &#10004;';
								} else {
									obj.textContent = txt;
					...
					{
						name:'Trung tâm đến con trỏ <span class="pk_shrtct">[Tab]</span>',
						action: function ( obj ) {
					...
					{
						name:'Đặt lại thu phóng <span class="pk_shrtct">[0]</span>',
						action: function ( obj ) {
			...
			},
			{
				...
				children:[
					{
						name   : 'Lưu trữ phiên bản ngoại tuyến',
						action : function () {
							if (window.location.href.indexOf('-cache') > 0) {

								function onUpdateReady ( e ) {
									if (confirm ('Bạn có muốn làm mới trang để tải phiên bản mới hơn không?'))
										window.location.reload();
								}
								function downLoading ( e ) {
									OneUp ('Đang tải xuống phiên bản mới hơn', 1500);
								}
							...
							var message = 'Thao tác này sẽ mở một cửa sổ mới sẽ cố gắng lưu trữ phiên bản cục bộ trong trình duyệt của bạn'; // nicer text

							new PKSimpleModal ({
								title : 'Mở phiên bản ngoại tuyến?',
						...
						setup: function ( obj ) {
							if (window.location.href.indexOf('-cache') > 0)
							{
								obj.innerHTML = 'Cập nhật phiên bản ngoại tuyến';
							}
						}
					},
					...
					{
						name   : 'Xem tin nhắn chào mừng',
						action : function () {
							PKAudioEditor._deps.Wlc ();
						}
					},
					// {
					// 	name   : 'Info LGV91 Editor Audio Mixer',
					// 	action : function () {
					// 		window.open ('/about.html');
					// 	}
					// },
...

					{
						name   : 'Ủng hộ',
						action : function () {
							window.open ('https://www.namls.name.vn/contact-us');
						}
					}
				]
			}
	...
	function _makeUIMainView ( UI, app ) {
		...
		app.listenFor ('DidChanToggle', function ( chan, val ) {
			if ( chan === 0) {
				if (val)
				{
					...
					btn_panner_left.innerHTML = '<strong>L</strong> BẬT';
				}
				else
				{
					btn_panner_left.classList.add ('pk_inact');
					btn_panner_left.innerHTML = '<strong>L</strong> TẮT';
				}
			} else {
				if (val)
				{
					btn_panner_right.classList.remove ('pk_inact');
					btn_panner_right.innerHTML = '<strong>R</strong> BẬT';
				}
				else
				{
					btn_panner_right.classList.add ('pk_inact');
					btn_panner_right.innerHTML = '<strong>R</strong> TẮT';
				}
			}
		});
		...
		btn_zoom_in_h.innerHTML = '+<span>Phóng to Horiz (+)</span>';
		...
		btn_zoom_out_h.innerHTML = '&ndash;<span>Thu nhỏ Horiz (-)</span>';
		...
		btn_zoom_reset.innerHTML = '[R] <span>Đặt lại thu phóng (0)</span>';
		...
		btn_zoom_in_v.innerHTML = '&#x2195; +<span>Phóng to theo chiều dọc</span>';
		...
		btn_zoom_out_v.innerHTML = '&#x2195; &ndash;<span>Thu nhỏ theo chiều dọc</span>';
		...
		ttmp.innerHTML = 'Kéo và thả Tệp âm thanh vào cửa sổ này hoặc nhấp vào ' +
		'<a style="white-space:nowrap;border:1px solid;border-radius:23px;padding:5px 18px;font-size:0.94em;margin-left:5px" '+
		'onclick="PKAudioEditor.engine.LoadSample()">ở đây để sử dụng một mẫu</a>';
		main_audio_view.appendChild( ttmp );

		var ttmp2 = d.createElement('div');
		ttmp2.className = 'pk_tmpMsg2';
		ttmp2.innerHTML = '<span>Vui lòng chờ...</span><div class="pk_mload"><div></div></div>' + 
	...
	function _makeUIToolbar (UI) {
		...
		btn_stop.innerHTML = '<span>Dừng phát lại (Space)</span>';
		...
		btn_play.innerHTML = '<span>Chơi (Space)</span>';
		...
		btn_pause.innerHTML = '<span>Tạm dừng (Shift+Space)</span>';
		...
		btn_loop.innerHTML = '<span>Chuyển đổi vòng lặp (L)</span>';
		...
		btn_back_jump.innerHTML = '<span>Tìm kiếm (left arrow)</span>';
		...
		btn_front_jump.innerHTML = '<span>Tìm kiếm (right arrow)</span>';
		...
		btn_back_total.innerHTML = '<span>Tìm kiếm sự bắt đầu (Shift + left arrow)</span>';
		...
		btn_front_total.innerHTML = '<span>Tìm kiếm kết thúc (Shift + right arrow)</span>';
		...
		btn_rec.innerHTML = '<span>Ghi (R)</span>';
		...
		setTimeout(function () {
			...
			main_context.addOption ('Chọn Chế độ xem hiển thị', function( e,x,i ) {
				UI.fireEvent ('RequestRegionSet');
			}, false );

			main_context.addOption ('Đặt lại thu phóng', function( e ) {
				UI.fireEvent ('RequestZoomUI', 0);
			}, false );

			main_context.addOption ('Đặt âm lượng/tăng', function( e ) {
				UI.fireEvent ('RequestFXUI_Gain');
			}, false );

			main_context.addOption ('Sap chép', function( e ) {
				var region = PKAudioEditor.engine.wavesurfer.regions.list[0];
				if (!region) return ;

				UI.fireEvent( 'RequestActionCopy');
			}, false );
			main_context.addOption ('Dán', function( e ) {
				if (!copable) return ;
				UI.fireEvent( 'RequestActionPaste');
			}, false );
			main_context.addOption ('Cắt', function( e ) {
				var region = PKAudioEditor.engine.wavesurfer.regions.list[0];
				if (!region) return ;

				UI.fireEvent( 'RequestActionCut');
			}, false );
			main_context.addOption ('Chèn im lặng', function( e ) {
		...
		copy_btn.innerHTML = '<span>Sao chép lựa chọn (Shift + C)</span>';
		...
		paste_btn.innerHTML = '<span>Dán lựa chọn (Shift + V)</span>';
		...
		cut_btn.innerHTML = '<span>Cắt lựa chọn (Shift + X)</span>';
		...
		silence_btn.innerHTML = '<span>Chèn im lặng (Shift + N)</span>';
		...
		selection.innerHTML = '<div class="pk_sellist">' + 
			'<span class="pk_title">Lựa chọn:</span>' + 
			'<div><span class="title">Bắt đầu:</span><span class="s_s pk_dat">-</span></div>' + 
			'<div><span class="title">Kết thúc:</span><span class="s_e pk_dat">-</span></div>' + 
			'<div><span  class="title">Khoảng thời gian:</span><span class="s_d pk_dat">-</span></div>' +
		'</div>';
		...
		btn_clear_selection.innerHTML = '<span>Xóa lựa chọn (Q key)</span>';
