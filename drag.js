(function( parent ) {
	...
	if( !window.FileReader || !document.addEventListener ) {
		throw( "API tệp không được hỗ trợ" );
		readFile = function(){ throw( "API tệp không được hỗ trợ" ); };
	}
	else {
		...
		readFile = function ( file, callback, method ) {
			...
			reader.onerror = function( e ) {
				var message,
					lut = [ "Không tìm thấy tập tin.", "Không thể mở tập tin",  
						"Không thể tải tệp lên", "Không thể đọc tệp", "Tệp quá lớn" ];
			...
			**/
			_overlayBuilder = function() {
				if( typeof overlay_id === "string" )
				{
					...
					tmp.innerHTML = "Kéo và thả tập tin!";
